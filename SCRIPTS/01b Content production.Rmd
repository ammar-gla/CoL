...
```{r, dates ,echo=FALSE}
  ...
  # Paye
  paye_d_month <- value_form(pull_dtpt(dt=paye_stats,flt='date_day == max(date_day) & geography_name=="London" & measure_name=="emp"',vr="d_change_mom"),s= signif_thous)
  
  paye_p_month <- perc_form(pull_dtpt(dt=paye_stats,flt='date_day == max(date_day) & geography_name=="London" & measure_name=="emp"',vr="p_change_mom"),d = digits_perc)
  ...
  paye_p_feb20 <- perc_form(pull_dtpt(dt=paye_stats,flt='date_day == max(date_day) & geography_name=="London" & measure_name=="emp"',vr="p_change_feb20"),d = digits_perc)
...
``` {r industry change, echo=FALSE, out.width='100%',fig.cap = figure_cap}
  ...
  lon_large_inds_paye <- paye_nuts1ind_stats %>%
    filter( geography_name=="London" & date_day == paye_ind_last_month & measure_name == "emp" & measure_value>=sector_thresh) %>% 
    pull(industry_name_simple)
  
  paye_ind_pay_feb20 <- paye_nuts1ind_stats %>%
    filter(date_day == paye_ind_last_month & measure_name == "median_pay" & industry_name_simple %in% lon_large_inds_paye & geography_name %in% c("London", "UK")) %>% 
    ...
    mutate(chart_ranking = case_when( 
      industry_name_simple == "Overall" & geography_name=="London" ~ 1,
      !(industry_name_simple == "Overall") & geography_name=="London" ~ dense_rank(desc(p_change_feb20))+1,
      TRUE ~ 99))  %>% 
    ...
    labs(title = paste0("Nominal median pay by industry, % change"),
         subtitle = paste0("Payrolled employee pay, between February 2020 and ", paye_ind_last_month_form),
         caption = "\nSource: HM Revenue and Customs – Pay As You Earn Real Time Information, ONS.\n\nNote: ")+
    theme(plot.caption = element_text(color = rgb(166,166,166,maxColorValue = 255)))
  ...
  LMU_plotly_settings(ch_name = paye_ind_pay_feb20,
                      ...
                      subtitle_text = 'paste0("Payrolled employee pay, between February 2020 and ", paye_ind_last_month_form)',
                      hover_mode = "closest") %>% 
...
``` {r industry change yoy, echo=FALSE, out.width='100%',fig.cap = figure_cap}
  ...
  paye_ind_pay_yoy <- paye_nuts1ind_stats %>%
    ...
    mutate(chart_ranking = case_when( 
      industry_name_simple == "Overall" & geography_name=="London" ~ 1,
      !(industry_name_simple == "Overall") & geography_name=="London" ~ dense_rank(desc(p_change_yoy))+1,
      TRUE ~ 99))  %>% 
    group_by(industry_name_simple) %>% 
    mutate(chart_ranking=min(chart_ranking)) %>% 
    ungroup() %>% 
    ...
    labs(title = paste0("Nominal median pay by industry, % change on year"),
         subtitle = paste0("Payrolled employee pay, ", paye_ind_last_month_form),
         caption = "\nSource: HM Revenue and Customs – Pay As You Earn Real Time Information, ONS.\n\nNote: ")+
    theme(plot.caption = element_text(color = rgb(166,166,166,maxColorValue = 255)))
  ...
  LMU_plotly_settings(ch_name = paye_ind_pay_yoy,
                      ...
                      subtitle_text = 'paste0("Payrolled employee pay, ", paye_ind_last_month_form)',
                      hover_mode = "closest") %>% 
    layout(xaxis = list(showgrid=TRUE),
           yaxis = list(showgrid=FALSE)) %>% 
    reverse_legend_labels()
...
``` {r industry pay level, echo=FALSE, out.width='100%',fig.cap = figure_cap}
  ...
  paye_ind_pay_level <- paye_nuts1ind_stats %>%
    filter(date_day == paye_ind_last_month & measure_name == "median_pay" & 
             industry_name_simple %in% lon_large_inds_paye & geography_name %in% c("London", "UK")) %>% 
    group_by(geography_name) %>% #To ensure chart ranking is correct, make rank based on London and assign within sector
    mutate(chart_ranking = case_when( 
      industry_name_simple == "Overall" & geography_name=="London" ~ 1,
      !(industry_name_simple == "Overall") & geography_name=="London" ~ dense_rank(desc(measure_value))+1,
      TRUE ~ 99))  %>% 
    group_by(industry_name_simple) %>% 
    mutate(chart_ranking=min(chart_ranking)) %>% 
    ungroup() %>% 
...
``` {r, paye ind pay trend, echo=FALSE, out.width='100%',fig.cap = figure_cap}
  ...
  paye_nuts1ind_bubble_stats <- paye_nuts1ind_stats %>% 
    ...
    mutate(measure_value_comp_month = measure_value[date_day==paye_ind_pay_comp_month],
           measure_value_latest = measure_value[date_day==paye_ind_last_month]) %>% 
    group_by(geography_name,date_day,measure_name) %>% 
    mutate(measure_share = measure_value/measure_value[industry_name_simple=="Overall"]) %>% 
    ungroup() %>% 
  ...
  paye_ind_pay_v_pay <- paye_nuts1ind_bubble_stats %>% 
    filter(date_day==paye_ind_last_month & geography_name=="London" & emp_measure_share>=0.01) %>% 
    ggplot() +
    geom_point(data = . %>% filter(industry_name_simple != "Overall"),
               aes(x=median_pay_measure_value_comp_month,
                   y=median_pay_p_change_feb20,
                   size=emp_measure_value,
                   text= paste0(industry_name_simple,"\n",
                                "Feb '20 pay level: £",value_form(median_pay_measure_value_comp_month),"\n",
                                "Pay growth: ",perc_form(100*median_pay_p_change_feb20),"%","\n",
                                "Employees: ",value_form(emp_measure_value))),
               color=pal,
               fill=pal,
               alpha=0.6)+
    scale_size(range = c(3, 15)) +
    geom_vline(data = . %>% filter(industry_name_simple == "Overall"),
               aes(xintercept = median_pay_measure_value_comp_month),
               size = 1 * mm_to_pt,
               colour = pal_lines) + # median london wage
    geom_hline(data = . %>% filter(industry_name_simple == "Overall"),
    ...
    labs(title =  paste0("Median pay level vs. median pay growth, ",paye_ind_pay_comp_month_form,", London"),
         subtitle = paste0("Growth to ",paye_ind_last_month_form,"; bubble size represents size of sector"),
         caption = paste0("\nSource: HM Revenue and Customs - Pay As You Earn Real Time Information.",
                          ...
                          "\nVertical line indicates level of median pay in London
                          in ",paye_ind_pay_comp_month_form,",",
                          "\nhorizontal line indicates change in London median pay ",paye_ind_pay_comp_month_form,"-",paye_ind_last_month_form))+
    theme(plot.caption = element_text(color = rgb(166,166,166,maxColorValue = 255)))
  
  # Add text labels separately as plotly does not support them
  paye_ind_pay_v_pay_static <- paye_ind_pay_v_pay +
    geom_text_repel(data = . %>% filter(industry_name_simple != "Overall"),
                    aes(x=median_pay_measure_value_comp_month,
                        y=median_pay_p_change_feb20,
  ...
  LMU_plotly_settings(ch_name = paye_ind_pay_v_pay,
                      ...
                      subtitle_text = 'paste0("Growth to ",paye_ind_last_month_form,"; bubble size represents size of sector")',
                      hover_mode = "closest")
...
paye_ind_emp_v_pay <- paye_nuts1ind_bubble_stats %>% 
  ...
  ggplot() +
  geom_point(data = . %>% filter(industry_name_simple != "Overall"),
             aes(x=emp_p_change_feb20,
                 y=median_pay_p_change_feb20,
                 size=emp_measure_value,
                   text= paste0(industry_name_simple,"\n",
                                "Employee growth: ",perc_form(100*emp_p_change_feb20),"%","\n",
                                "Pay growth: ",perc_form(100*median_pay_p_change_feb20),"%","\n",
                                "Employees: ",value_form(emp_measure_value))),
             color=pal,
             fill=pal,
  ...
  scale_size(range = c(3, 15)) +
  geom_vline(data = . %>% filter(industry_name_simple == "Overall"),
             aes(xintercept = emp_p_change_feb20),
             size = 1 * mm_to_pt,
             colour = pal_lines) + # median london wage
  geom_hline(data = . %>% filter(industry_name_simple == "Overall"),
             aes(yintercept = median_pay_p_change_feb20),
  ...
  labs(title =  paste0("Employee growth vs. median pay growth, London"),
       subtitle = paste0("Growth from ",paye_ind_pay_comp_month_form, " to ",paye_ind_last_month_form ,"; bubble size represents size of sector"),
       caption = paste0("\nSource: HM Revenue and Customs - Pay As You Earn Real Time Information.",
                        "\n\nNote: Estimates are based on where employees live. Excludes sectors with less than 1% of workforce.",
                        "\nVertical line indicates change in London employees ",paye_ind_pay_comp_month_form,"-",paye_ind_last_month_form,",",
                        "\nhorizontal line indicates change in London median pay ",paye_ind_pay_comp_month_form,"-",paye_ind_last_month_form))+
  theme(plot.caption = element_text(color = rgb(166,166,166,maxColorValue = 255)))
...
# Save chart
save_GLA_plot(plot_name = "paye_ind_emp_v_pay_static",w=10, h=7)

  
  # Plotly chart
  LMU_plotly_settings(ch_name = paye_ind_emp_v_pay,
                      title_text = 'paste0("Employee growth vs. median pay growth, London")',
                      subtitle_text = 'paste0("Growth from ",paye_ind_pay_comp_month_form, " to ",paye_ind_last_month_form ,"; bubble size represents size of sector")',
                      hover_mode = "closest")
...
``` {r la change, echo=FALSE, out.width='100%',fig.cap = figure_cap}
  ...
  # Run static map
  paye_la_pay_feb20_map <- ggplot_map_output(dataset=paye_la_stats_pay_geo,
                                       ...
                                       perc_bin = TRUE,
                                       chart_name="paye_la_pay_map",
                                       caption_txt=figure_cap)
  
  # Run interactive map and output to markdown
  leaflet_map_output(dataset=paye_la_stats_pay_geo,
                     dt_var="p_change_feb20",
                     bin_rounding = 2,
                     diverge_scale = FALSE,
                     pal_colour ="blue",
...
``` {r la change yoy, echo=FALSE, out.width='100%',fig.cap = figure_cap}
  ...
  # Run static map
  paye_la_pay_yoy_map <- ggplot_map_output(dataset=paye_la_stats_pay_geo,
                                       dt_var="p_change_yoy",
                                       bin_rounding = 2,
                                       diverge_scale = FALSE,
                                       pal_colour = "blue",
...
``` {r la pay level, echo=FALSE, out.width='100%',fig.cap = figure_cap}
  ...
  # Run static map
  paye_la_pay_level_map <- ggplot_map_output(dataset=paye_la_stats_pay_geo,
...
``` {r, paye LA pay trend, echo=FALSE, out.width='100%',fig.cap = figure_cap}
  ...
  paye_la_pay_v_pay <- paye_la_bubble_stats %>% 
    filter(date_day==paye_la_last_month & geography_name!="UK") %>% 
    ggplot() +
    ...
    labs(title =  paste0("Median pay level vs. median pay growth, ",paye_la_pay_comp_month_form,", London boroughs"),
         subtitle = paste0("Growth to ",paye_ind_last_month_form,"; bubble size represents employees in borough"),
         caption = paste0("\nSource: HM Revenue and Customs - Pay As You Earn Real Time Information.",
                          "\n\nNote: Estimates are based on where employees live. Excludes City of London.",
                          "\nVertical line indicates level of median pay in London in ",paye_la_pay_comp_month_form,",","\nhorizontal line indicates change in London median pay ",paye_la_pay_comp_month_form,"-",paye_ind_last_month_form))+
    theme(plot.caption = element_text(color = rgb(166,166,166,maxColorValue = 255)))
  ...
  LMU_plotly_settings(ch_name = paye_la_pay_v_pay,
                      ...
                      subtitle_text = 'paste0("Growth to ",paye_ind_last_month_form,"; bubble size represents employees in borough")',
                      hover_mode = "closest")
  
  figure_cap <- paste0("Source: HM Revenue and Customs - Pay As You Earn Real Time Information.",
                          "<br>","<br>","Note: Estimates are based on where employees live. Excludes City of London.","<br>","Vertical line indicates level of median pay in London in ",paye_ind_pay_comp_month_form,",","<br>","horizontal line indicates change in London median pay ",paye_ind_pay_comp_month_form,"-",paye_ind_last_month_form)
...
# Bubbles are size of industry employees, values are growth vs. level in pay
  ...
  paye_la_emp_v_pay <- paye_la_bubble_stats %>% 
    ...
    labs(title =  paste0("Employee growth vs. median pay growth, London boroughs"),
         subtitle = paste0("Growth from ",paye_la_pay_comp_month_form, " to ",paye_la_last_month_form ,"; bubble size represents employees in borough"),
         caption = paste0("\nSource: HM Revenue and Customs - Pay As You Earn Real Time Information.",
                          "\n\nNote: Estimates are based on where employees live. Excludes City of London.",
                          "\nVertical line indicates level of median pay in London in ",paye_la_pay_comp_month_form,",","\nhorizontal line indicates change in London median pay ",paye_la_pay_comp_month_form,"-",paye_ind_last_month_form))+
    theme(plot.caption = element_text(color = rgb(166,166,166,maxColorValue = 255)))
  ...
  # Save chart
  save_GLA_plot(plot_name = "paye_la_emp_v_pay_static",w=10, h=7)
  
  # Plotly chart
  LMU_plotly_settings(ch_name = paye_la_emp_v_pay,
                      title_text = 'paste0("Employee growth vs. median pay growth, London boroughs")',
                      subtitle_text = 'paste0("Growth from ",paye_la_pay_comp_month_form, " to ",paye_la_last_month_form ,"; bubble size represents employees in borough")',
                      hover_mode = "closest")
...
``` {r age group feb20, echo=FALSE, out.width='100%',fig.cap = figure_cap}
  ...
  paye_age_pay_feb20 <- paye_nuts1age_stats %>%
    ...
    filter( date_day == paye_age_last_month & !(age_group %in% c("Aged: 0-17")) & geography_name %in% c("London", "UK") & measure_name=="median_pay" ) %>%  
    ...
    labs(title = "Nominal median pay by age group, % change",
         subtitle = paste0("Payrolled employee pay, between February 2020 and ", paye_age_last_month_form),
         caption = "\nSource: HM Revenue and Customs – Pay As You Earn Real Time Information.\n\nNote: Estimates are based on where employees live.") +
    theme(plot.caption = element_text(color = rgb(166,166,166,maxColorValue = 255)))
  ...
  LMU_plotly_settings(ch_name = paye_age_pay_feb20,
                      ...
                      subtitle_text = 'paste0("Payrolled employee pay, between February 2020 and ", paye_age_last_month_form)',
                      hover_mode = "closest") 
...
``` {r age group yoy, echo=FALSE, out.width='100%',fig.cap = figure_cap}
  ...
  paye_age_pay_feb20 <- paye_nuts1age_stats %>%
    ...
    labs(title = "Nominal median pay by age group, % change on year",
         subtitle = paste0("Payrolled employee pay, ", paye_age_last_month_form),
         caption = "\nSource: HM Revenue and Customs – Pay As You Earn Real Time Information.\n\nNote: Estimates are based on where employees live.") +
    theme(plot.caption = element_text(color = rgb(166,166,166,maxColorValue = 255)))
  ...
  LMU_plotly_settings(ch_name = paye_age_pay_feb20,
                      ...
                      subtitle_text = 'paste0("Payrolled employee pay, ", paye_age_last_month_form)',
                      hover_mode = "closest") 
...
``` {r age group pay level, echo=FALSE, out.width='100%',fig.cap = figure_cap}
  ...
  ggplotly(paye_age_pay_level,tooltip = "text")    %>% 
    ...
    layout(title = list(text = paste0("<b>","Nominal median pay by age group, level (£)","</b>",
                                      "<br>",
                                      "<sup>",
                                      paste0("Payrolled employee pay, latest data for ", paye_age_last_month_form),
                                      "</sup>",
                                      "<br>"),
                        font = list(size = 22),
                        y = .95, xref = "plot"),
...
``` {r nuts2 feb2020, echo=FALSE, out.width='100%'}
  ...
  paye_nuts2_pay_feb20 <- paye_nuts2_stats %>%
    ...
    labs(title =  "Nominal median pay by NUTS2 region, % change",
         subtitle = paste0("Payrolled employees, between February 2020 and ", paye_overall_last_month_form),
         caption = "\nSource: HM Revenue and Customs – Pay As You Earn Real Time Information.\n\nNote: Estimates are based on where employees live.") + 
    theme(legend.position = "none",
...
``` {r nuts2 pay comp yoy, echo=FALSE, out.width='100%'}
  ...
  paye_nuts2_pay_yoy <- paye_nuts2_stats %>%
    ...
    labs(title =  "Nominal median pay by NUTS2 region, % change on year",
         subtitle = paste0("Payrolled employees, ", paye_overall_last_month_form),
         caption = "\nSource: HM Revenue and Customs – Pay As You Earn Real Time Information.\n\nNote: Estimates are based on where employees live.") + 
    theme(legend.position = "none",
          plot.caption = element_text(color = rgb(166,166,166,maxColorValue = 255)))
