  ...
  leaflet_map_output <- function(dataset,
    ...
    # Create the palette
    pal_func <- colorBin(palette, domain = dataset[[dt_var]], bins = bins)
                                ...
                                title)
  {
    ...
    checkmate::assert_choice(ilo_stat, choices = c("Unemployment rate - aged 16-64","% who are economically inactive - aged 16-64" ))
    
    # Need to create floor, ceiling and interval variables in data
    floor_var <- (paste0("d_rate_",year_from,"_floor"))
    int_var <- (paste0("d_rate_",year_from,"_interval"))
    date_to <- paste0(year_to,"-12-01")
    
    range_vec <- aps_stats_geo %>%
      filter(date_day==as.Date(date_to) & variable_name==ilo_stat) %>%
      distinct(!!sym(floor_var),!!sym(int_var)) %>%
      arrange(!!sym(floor_var)) %>%  #NA will be last so make it grey
      mutate(!!sym(int_var):=replace(!!sym(int_var), str_detect(!!sym(int_var), "NA"), "NA"))
    
    # The below could be used for a dynamic map with both decreasing and inreasing values
    # n_intervals_dec <- sum(range_vec %>% pull(!!sym(floor_var))<0,
    #                        na.rm = TRUE) #since we want diverging scale around 0
    # n_intervals_inc <- sum(range_vec %>% pull(!!sym(floor_var))>=0,
    #                        na.rm = TRUE)
    # 
    # pal <- c(gla_pal(palette_type = "quantitative",n=n_intervals_dec,main_colours = "blue",remove_margin="both"),
    #          rev(gla_pal(palette_type = "quantitative",n=n_intervals_inc,main_colours = "red",remove_margin="both")),
    #          "grey")
    # 
    # range_names <- range_vec %>% pull(!!sym(int_var))
    # pal_named <- setNames(pal,nm=range_names)
    
    la_map_rates <- aps_stats_geo %>%
      filter(date_day==date_to & variable_name==ilo_stat) %>%
      ggplot(aes(geometry = geometry,
                 fill = factor(!!sym(int_var),levels=range_names))) +
      ggla_sf() +
      scale_fill_manual(values = pal_named) +
      labs(
        title = title,
        subtitle = paste0("Percentage point change between Jan-Dec ",year_from," and Jan-Dec ",year_to),
        caption = paste0("Source: APS, 12-month data on 3-month rolling basis.\nContains Ordnance Survey data Crown copyright and database right [2015]",
                         "\n\nNotes: Unemployment rate as share of economically active population aged 16-64."))
    
    save_GLA_plot(plot_name = chart_name,path = paste0(IMAGES,"overall/"),h=8,w=12)
  }
