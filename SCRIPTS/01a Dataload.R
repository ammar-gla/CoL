  ...
  save_GLA_plot <- function(plot_name, w=8, h=8) {
    ...
    ggsave(paste0(IMAGES,plot_name,".png"), device = "png", width = w, height = h, units = "in")
  }
  ...
  correct_facets <- function(chart,max_fac_num=9) {
    ...
    for (fac_num in 1:max_fac_num) {
      ...
      # Depending on number of facets, set y position differently
      if (between(max_fac_num,6,8)) {
      ...
      else if (between(max_fac_num,9,12)) {
        if (fac_num %in% c(1,2,3)) {
          y_annot <- 1
          ydom <- c(0.73,0.99)
        }          else if (fac_num %in% c(4,5,6)) {
  ...
  map_prep <- function(dataset,
    ...
    # Check certain parameters are specified
    checkmate::assertNumber(bin_rounding)
    checkmate::assertCharacter(dt_var)
    checkmate::assertTRUE(!is.null(pal_colour))
    
    # Further modification of tooltip in leaflet
    if (is.null(tooltip_txt)) tooltip_txt <- "Change: " #default is changes
    
    if (perc_txt=="%") {
      if (is.null(decimal_n)) decimal_n <- 1 #default
      if (is.null(sig_figures)) sig_figures <- 3 # this is not actually used
    }
    else {
      if (is.null(decimal_n)) decimal_n <- 0 #default
      if (is.null(sig_figures)) sig_figures <- 3 #default
    }
    
    # Create interval ranges for bins
    dataset_bins <- dataset %>%
      arrange(!!sym(dt_var)) %>% 
      mutate(tooltip= case_when(perc_txt=="%" ~ paste0(tooltip_txt, perc_form(!!sym(dt_var),d=decimal_n),perc_txt),
                                perc_txt=="" ~ paste0(tooltip_txt, value_form(!!sym(dt_var),d=decimal_n,s=sig_figures),perc_txt)),
             value_floor = rounder(!!sym(dt_var),-bin_rounding),
             value_ceil = rounder(!!sym(dt_var),bin_rounding),
             value_interval = paste0(value_floor,perc_txt," to ",value_ceil,perc_txt)) 
    ...
    bin_data <- dataset_bins %>% 
      ...
      arrange(bin_val) %>% 
      distinct(bin_val) %>% 
      mutate(bin_int = paste0(bin_val,perc_txt," to ",lead(bin_val,n=1),perc_txt),
  ...
  leaflet_map_output <- function(dataset,
                                 ...
                                 pal_colour,
                                 title,
                                 ...) #allow to pass further arguments to prep function
  {
    ...
    # Create the palette
    pal_func <- colorBin(palette, domain = dataset_bins[[dt_var]], bins = map_bins)
    
    # Make tool tip
    labels <- sprintf("<strong>%s</strong><br/>%s",dataset_bins$geography_name, dataset_bins$tooltip) %>%
      lapply(htmltools::HTML)
    
    # Make map
    map_output1 <- leaflet(dataset_bins) %>%
      addPolygons(fillColor = ~pal_func(dataset_bins[[dt_var]]),
                  weight = 1,
                  opacity = 1,
                  color = "white",
                  fillOpacity = 0.8,
  ...
  paye_pay_aux <- paye_pay_stats %>% 
    pivot_longer(cols = c("UK","London"),names_to = "geography_name",values_to = "measure_value") %>% 
    rename(Overall = measure_value) 
    
  remove(paye_emp_stats) #do not remove paye_pay_stats as needed for CPIH below
  
  paye_overall_last_month <- max(paye_stats$date_day)
  paye_overall_last_month_form <- format(paye_overall_last_month,"%B %Y")
  ...
  paye_age_emp_uk <- readxl::read_excel(path = paste0(INTERMEDIATE,latest_paye_nuts1age_name), sheet = "28. Employees (Age)", skip = 5) %>%
    mutate(date_day = dmy(paste0("01",Date)),
           measure_name = "emp",
           geography_name = "UK",
           Overall = rowSums(across(c("0-17","18-24","25-34","35-49","50-64","65+")))) %>% 
    select(date_day,geography_name, measure_name, "0-17","18-24","25-34","35-49","50-64","65+",Overall) 
  ...
  paye_nuts1age_stats <- paye_nuts1age_emp %>%
    ...
    pivot_longer(cols="0-17":"Overall",names_to = "age_group", values_to = "measure_value") %>% 
    ...
    mutate(p_change_feb20 = case_when( date_day <= as.Date("2020-02-01") ~ NaN,
           ...
           index_feb20 = 100*measure_value/measure_value[date_day==as.Date("2020-02-01")],
           age_group = case_when( age_group=="Overall" ~ age_group,
  ...
  paye_age_last_month <- max(paye_nuts1age_stats$date_day)
  paye_age_last_month_form <- format(paye_age_last_month,"%B %Y")
  ...
  paye_ind_emp_uk <- readxl::read_excel(path = paste0(INTERMEDIATE,latest_paye_nuts1ind_name), sheet = "23. Employees (Industry)", skip = 6) %>% 
    mutate(date_day = dmy(paste0("01",Date)),
           geography_name="UK",
           measure_name = "emp",
           Overall = rowSums(across(c("Agriculture, forestry and fishing":"Households and Extraterritorial")))) %>% 
    select(-c("UK"))
  ...
  paye_nuts1ind_emp <- readxl::read_excel(path = paste0(INTERMEDIATE,latest_paye_nuts1ind_name), sheet = "36. Employees (NUTS1Sector)", skip = 6) %>% 
    mutate(date_day = dmy(paste0("01",Date)),
           geography_name="London",
           measure_name = "emp",
           Overall = rowSums(across(c("London: Agriculture, forestry and fishing":"London: Households and Extraterritorial")))) %>% 
    select(c(Date,date_day,geography_name,contains("London"),measure_name,Overall))  %>%
    rename_with(~str_replace(.x,pattern="London: ",replacement=""))
  ...
  paye_nuts1ind_stats <- paye_nuts1ind_emp %>% 
    ...
    pivot_longer(cols=c("Agriculture, forestry and fishing":"Households and Extraterritorial","Overall"),
  ...
  paye_ind_last_month <- max(paye_nuts1ind_stats$date_day)
  paye_ind_last_month_form <- format(paye_ind_last_month,"%B %Y")
  ...
  paye_la_stats <-  paye_la_emp_stats %>% 
    ...
    mutate( 
      p_change_feb20 = case_when( date_day <= as.Date("2020-02-01") ~ NaN,
                                  date_day > as.Date("2020-02-01") ~ 
                                    (measure_value - measure_value[date_day == "2020-02-01"])/(measure_value[date_day == "2020-02-01"])),
      d_change_mom = case_when(date_day - min(date_day) < months(1) ~ NaN,
                               date_day - min(date_day) >= months(1) ~ 
                                 (measure_value - lag(measure_value, n = 1))),
      p_change_mom = case_when(date_day - min(date_day) < 365 ~ NaN,
  ...
  remove(paye_la_emp_stats,paye_la_pay_stats)
  
  paye_la_last_month <- max(paye_la_stats$date_day)
  paye_la_last_month_form <- format(paye_la_last_month,"%B %Y")
  ...
  #   rename(date_month=date,measure_value=obs_value) %>% 
  ...
  #   mutate( 
  #     change_p_dec19 = case_when( date_day <= as.Date("2019-12-01") | measures_name == "Percent" ~ NaN,
  #                                          date_day > as.Date("2019-12-01") ~ 100*(measure_value - measure_value[ date_day == "2019-12-01"])/(measure_value[ date_day == "2019-12-01"])),
  #     change_d_dec19 = case_when( date_day <= as.Date("2019-12-01") | measures_name == "Percent" ~ NaN,
  #                                     date_day > as.Date("2019-12-01") ~ (measure_value - measure_value[ date_day == "2019-12-01"])),
  #     change_p_quar = case_when(  measures_name == "Percent" ~ NaN,
  #                                    TRUE ~ 100*(measure_value - lag(measure_value, n = 1))/(lag(measure_value, n = 1))),
  #     change_d_quar = case_when(  measures_name == "Percent" ~ NaN,
  #                               TRUE ~ (measure_value - lag(measure_value, n = 1))),
  #     industry_name = case_when(industry_name == "Overall" ~ "Overall",
  ...
  #   mutate(change_p_dec19_within_date_rank = case_when( industry_name == "Overall" ~ NA_integer_, 
  #                                                          TRUE  ~ dense_rank(desc(change_p_dec19))), 
  #          change_p_quart_within_date_rank = case_when( industry_name == "Overall" ~ NA_integer_, 
  #                                                          TRUE  ~ dense_rank(desc(change_p_quar)))) %>% 
  ...
  cpih_stats <- read.csv(paste0(INTERMEDIATE,Sys.Date(),"_raw_cpih.csv"), header=FALSE, stringsAsFactors=FALSE) %>% 
    ...
    mutate(cpih_yoy=as.numeric(cpih_yoy)/100,
           ...
           cpih_annualised_1m = (cpih_mom_help)^12-1,
    ...
    mutate(cpih_yearsofar_help = cumprod(cpih_mom_help),
           cpih_annualised_ytd = cpih_yearsofar_help^(12/month)-1) %>% 
    ungroup()
  ...
  cpih_full_year <- tibble( # Create dataset with future months
    ...
    fill(c(cpih_annualised_ytd,cpih_index_2015), .direction="down") %>% 
    mutate(cpih_annualised_mom = (cpih_annualised_ytd+1)^(1/12),
  ...
  cpih_pay_stats <- paye_pay_stats %>%
    ...
    mutate(p_change_yoy = case_when(date_day - min(date_day) < 365 ~ NaN,
           ...
           p_annualised_1m = (1+p_change_mom)^12-1,
  ...
  paye_la_stats_pay_geo <- read_sf(here("INPUT","statistical-gis-boundaries-london","ESRI","London_Borough_Excluding_MHW.shp")) %>% 
    clean_names() %>% 
    ...
    mutate(across(contains("p_change"), ~ .x*100)) %>% 
    st_transform(4326)
